...
const FAQ = () => {
    ...
    try {
      ...
      if (response.success) {
        Swal.fire({
          ...
          text: isArabic
            ? 'سؤالك تم إرساله بنجاح وسيتم الرد عليك قريباً'
      ...
      Swal.fire({
        ...
        text: isArabic
          ? 'لم نتمكن من إرسال سؤالك. يرجى المحاولة مرة أخرى.'
  ...
  if (loading) {
    return (
      <div className="faq-container">
        {/* SEO بدون faqItems لأن البيانات لم تُحمَّل بعد */}
        <SEO
          page="faq"
          breadcrumbItems={[
            { name: isArabic ? 'الرئيسية' : 'Home', url: '/' },
            { name: isArabic ? 'الأسئلة الشائعة' : 'FAQ', url: '/faq' }
          ]}
        />
    ...
    return (
      <div className="faq-container">
        ...
        <div className="faq-error">
          <p>
            {isArabic
              ? 'عذراً، حدث خطأ في تحميل الأسئلة الشائعة'
              : 'Sorry, error loading FAQ'}
          </p>
          <button className="retry-btn" onClick={fetchFaqData}>
  ...
  // بناء faqItems للـ FAQ schema بعد ما البيانات جاهزة
  const seoFaqItems = questions.map((q) => ({
    question: q.question,
    answer: q.answer
  }));
    ...
    <div className="faq-container">
      ...
      >
        ...
        <p className="faq-subtitle">
          {section?.subtitle ||
            (isArabic
              ? 'كل ما تحتاج معرفته عن رحلتك الرياضية ❤️'
              : 'Everything you need to know about your fitness journey 🤍')}
        </p>
      ...
      >
        ...
        <div className="stat-item">
          ...
          <span className="stat-label">{isArabic ? 'دعم متاح' : 'Support Available'}</span>
        </div>
        <div className="stat-item">
          <span className="stat-number">100%</span>
          <span className="stat-label">{isArabic ? 'معدل الاستجابة' : 'Response Rate'}</span>
        </div>
      </motion.div>
      ...
      <div className="faq-content">
        {searchQuery.trim() !== '' && (
          ...
          >
            <p>
              {isArabic
                ? `تم العثور على ${filteredQuestions.length} نتيجة`
                : `Found ${filteredQuestions.length} results`}
              {searchQuery && (
                <span>
                  {' '}
                  {isArabic ? 'للبحث:' : 'for:'} &quot;{searchQuery}&quot;
                </span>
              )}
            </p>
          </motion.div>
        ...
        >
          <div className="form-header">
            ...
            <p>
              {isArabic
                ? 'اسألنا أي شيء! نحن هنا لمساعدتك على النجاح'
                : "Ask us anything! We're here to help you succeed"}
            </p>
          </div>
      ...
      <div className="faq-decorations">
        {[...Array(8)].map((_, i) => (
          <motion.div
            ...
            animate={{ y: [0, -30, 0], opacity: [0.2, 0.5, 0.2], rotate: [0, 360] }}
