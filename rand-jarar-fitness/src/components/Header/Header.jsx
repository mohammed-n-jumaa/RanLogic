...
const Header = () => {
  ...
  const handleLoginClick = () => {
    if (user) {
      navigate(user.has_active_subscription ? '/profile' : '/plans');
    } else {
      setShowAuth(true);
      setMenuOpen(false);
    }
  ...
  const pageLinks = currentLang === 'ar'
    ? [
        ...
        { name: 'حاسبة الوجبة',    path: '/meal-calculator' },
      ]
    : [
        { name: 'Home',               path: '/' },
        { name: 'FAQ',                path: '/faq' },
        { name: 'Calorie Calculator', path: '/calorie-calculator' },
        { name: 'Meal Calculator',    path: '/meal-calculator' },
      ];

  // "عن الفريق" بدل "عن المدرب"
...
<nav className="nav desktop-nav">
  {pageLinks
    .filter(link => !(isHomePage && (link.path === '/')))  // إخفاء "الرئيسية" عند وجودنا فيها
    .map((link) => (
              ...
              <div className="user-dropdown-container desktop-only" ref={userDropdownRef}>
                ...
                >
                  <img
                    ...
                    height="40"
                    loading="lazy"
                    style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover', aspectRatio: '1/1' }}
