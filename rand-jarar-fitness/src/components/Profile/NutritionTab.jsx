...
const NutritionTab = () => {
  ...
  const fetchNutritionPlan = async () => {
    try {
      setLoading(true);
      const now = new Date();
      const response = await profileApi.getMyNutritionPlan(
        now.getFullYear(),
        now.getMonth() + 1
      );
  ...
  const getCurrentPeriodDays = () => {
    ...
    for (let day = startDay; day <= endDay; day++) {
      ...
      days.push({
        day: day,
        date: date.toLocaleDateString(t('ar-SA', 'en-US'), {
          month: 'short',
          day: 'numeric'
        }),
        meals: dayMeals.map((meal) => ({
          ...
          protein:
            meal.items?.reduce((sum, item) => sum + (item.protein || 0), 0) || 0,
          carbs:
            meal.items?.reduce((sum, item) => sum + (item.carbs || 0), 0) || 0,
          fats:
            meal.items?.reduce((sum, item) => sum + (item.fats || 0), 0) || 0,
  ...
  const hasPlan = !!nutritionPlan?.plan;
  const hasMeals = !!nutritionPlan?.meals?.length;

  const periodDays = getCurrentPeriodDays();
  const currentDayData = periodDays[selectedDay - 1] || {
    meals: [],
    totalCalories: 0,
    protein: 0,
    carbs: 0,
  ...
  return (
    <div className="nutrition-tab empty">
      <div className="empty-state">
        ...
        <p>
          {t(
            'لم يتم إنشاء خطة تغذية لهذا الشهر بعد',
            'No nutrition plan has been created for this month yet'
          )}
        </p>
      </div>
    </div>
    ...
    return (
      <div className="nutrition-tab empty">
        <div className="empty-state">
          ...
          <p>
            {t(
              'لا توجد وجبات مضافة داخل النظام حالياً، يمكنك عرض أو تحميل ملف الخطة الشهرية.',
              'There are no meals added in the system currently, but you can view or download the monthly plan PDF.'
            )}
          </p>
