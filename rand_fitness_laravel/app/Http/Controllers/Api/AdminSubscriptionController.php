    ...
    public function getPlans(Request $request): JsonResponse
{
    try {
        ...
        $plans = $dbPlans->map(function (Plan $plan) use ($isArabic, $geoService, $rate) {
            ...
            // Build pricing in USD first, then convert
            $pricingUsd = [
                '1month'  => [
            ...
            // Convert to local currency via geo service
            $pricing = $geoService->convertPlanPricing($pricingUsd, $rate);

            // Keep USD prices as reference (needed for PayPal — always USD)
            ...
            return [
                ...
                'features' => $isArabic
                    ? (is_array($plan->features_ar) ? $plan->features_ar : json_decode($plan->features_ar, true) ?? [])
                    : (is_array($plan->features_en) ? $plan->features_en : json_decode($plan->features_en, true) ?? []),
                'color'    => $plan->color,
                'icon'     => $plan->icon,
            ];
        });
