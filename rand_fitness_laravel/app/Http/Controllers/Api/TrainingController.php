...
public function saveWorkoutPlan(int $userId, Request $request): JsonResponse
{
    DB::beginTransaction();
    try {
        Log::info('=== START saveWorkoutPlan ===');
        Log::info('User ID: ' . $userId);
        Log::info('All request data keys: ' . json_encode(array_keys($request->all())));
        Log::info('All files keys: ' . json_encode(array_keys($request->allFiles())));
        
        // ===== فحص وجود ملف PDF =====
        Log::info('Has pdf_file? ' . ($request->hasFile('pdf_file') ? 'YES' : 'NO'));
        ...
        $validator = Validator::make($request->all(), [
            ...
            'pdf_file.max'   => 'حجم PDF يجب ألا يتجاوز 10MB',
        ]);
        ...
        // ===== Handle PDF file - بنفس طريقة النظام الغذائي =====
        if ($request->hasFile('pdf_file')) {
            $pdfFile = $request->file('pdf_file');
            
            if ($pdfFile->isValid()) {
                $data['pdf_file'] = $pdfFile;
                Log::info('PDF file attached to data successfully');
            } else {
                Log::error('PDF file is not valid, error code: ' . $pdfFile->getError());
                return response()->json([
                    'success' => false,
                    'message' => 'ملف PDF غير صالح',
                ], 422);
            }
        } else {
        ...
        $videoCount = 0;
        foreach ($request->allFiles() as $key => $file) {
            if (str_starts_with($key, 'video_')) {
                Log::info("Found exercise video: {$key}");
                $data[$key] = $file;
                $videoCount++;
            }
        }
        Log::info("Total video files found: {$videoCount}");
        ...
        Log::info('=== END saveWorkoutPlan - SUCCESS ===');
        ...
        DB::rollBack();
        Log::error('=== Error saving workout plan ===');
        Log::error('Message: ' . $e->getMessage());
        Log::error('File: ' . $e->getFile() . ':' . $e->getLine());
        Log::error('Trace: ' . $e->getTraceAsString());
...
public function importWorkoutExcel(int $userId, Request $request): JsonResponse
{
    DB::beginTransaction();
    try {
        Log::info('=== START importWorkoutExcel ===');
        Log::info('User ID: ' . $userId);
        ...
        $result = $this->trainingService->importExercisesFromExcel(
            $userId,
        ...
        return response()->json([
            'success' => true,
            'message' => "تم استيراد {$result['imported']} تمرين بنجاح" . $errorDetails,
            'data' => [
